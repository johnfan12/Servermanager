...
SERVERMANAGER_PORT="${SERVERMANAGER_PORT:-${FRP_API_LOCAL_PORT:-${FRP_API_REMOTE_PORT:-}}}"
if [ -z "$SERVERMANAGER_PORT" ]; then
  echo "[start.sh] missing port config; please set SERVERMANAGER_PORT or FRP_API_LOCAL_PORT or FRP_API_REMOTE_PORT in .env"
  exit 1
fi
export SERVERMANAGER_PORT

FRP_API_ENABLED="${FRP_API_ENABLED:-true}"
FRP_API_REMOTE_PORT="${FRP_API_REMOTE_PORT:-$SERVERMANAGER_PORT}"
